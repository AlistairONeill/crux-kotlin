...
class Transactions {
    ...
    @Nested
    inner class PuttingICruxDB {
        @Test
        fun `Autogenerated keywords`() {
            data class Person(
            ...
            val pabloOut = node.db().use { it.entity(pabloId) }

            assertEquals(8, pabloOut.size)
            assertEquals("Pablo", pabloOut["person/tString".kw])
            assertEquals(pabloId, pabloOut["person/tKw".kw])
            assertEquals(1, pabloOut["person/tInt".kw])
            assertEquals(2L, pabloOut["person/tLong".kw])
            assertEquals(3f, pabloOut["person/tFloat".kw])
            assertEquals(4.0, pabloOut["person/tDouble".kw])
            assertEquals(true, pabloOut["person/tBool".kw])
        }

        @Test
        fun `Manually specifying keywords`() {
            data class Person(
                override val cruxId: Keyword,
                @property:CruxKey("myString") val tString: String,
                @property:CruxKey("myKeyword") val tKw: Keyword,
                @property:CruxKey("myInt") val tInt: Int,
                @property:CruxKey("myLong") val tLong: Long,
                @property:CruxKey("myFloat") val tFloat: Float,
                @property:CruxKey("myDouble") val tDouble: Double,
                @property:CruxKey("myBoolean") val tBool: Boolean
            ): ICruxDataClass
            ...
            assertEquals(8, pabloOut.size)
            assertEquals("Pablo", pabloOut["myString".kw])
            assertEquals(pabloId, pabloOut["myKeyword".kw])
            assertEquals(1, pabloOut["myInt".kw])
            assertEquals(2L, pabloOut["myLong".kw])
            assertEquals(3f, pabloOut["myFloat".kw])
            assertEquals(4.0, pabloOut["myDouble".kw])
            assertEquals(true, pabloOut["myBoolean".kw])
        }
    }
